...
// - loaded from a local checkout (src/plugin-opencode/daemon-launcher.ts -> src/daemon/index.ts)
...
function findRunner(): Runner | undefined {
  ...
  if (nodePath && tsxModule)
    return { command: nodePath, args: ["--import", tsxModule] };
