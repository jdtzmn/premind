...
type ToolDefinition = {
  name: string
  execute: (toolCallId: string, params: Record<string, never>, signal?: AbortSignal, onUpdate?: unknown, ctx?: unknown) => Promise<ToolResult>
}
...
const createExtension = (options: { pruneResult?: { sessions: number; reminderBatches: number } } = {}) =>
  createPremindPiExtension({
    createDaemonClient: () => ({
      debugStatus: async () => status,
      pruneClosedSessions: async () => options.pruneResult ?? { sessions: 0, reminderBatches: 0 },
    }),
  })

describe("premind Pi extension", () => {
  test("registers namespaced commands and status tool", () => {
    const mock = createMockPi()
    createExtension()(mock.pi as never)
  ...
  test("/premind:status renders daemon status", async () => {
    ...
    assert.match(notifications[0]?.message ?? "", /owner\/repo @ feature\/pi \(PR #123\)/)
  })

  test("/premind:prune prunes closed sessions", async () => {
    const mock = createMockPi()
    const notifications: Array<{ message: string; level: string }> = []
    createExtension({ pruneResult: { sessions: 401, reminderBatches: 17 } })(mock.pi as never)
    ...
    assert.equal(notifications.length, 1)
    assert.equal(notifications[0]?.level, "info")
    assert.match(notifications[0]?.message ?? "", /premind pruned 401 closed sessions and 17 reminder batches\./)
  })
