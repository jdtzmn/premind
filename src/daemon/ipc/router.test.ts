...
describe("generic reminder claim IPC", () => {
  ...
  test("prevents a generic claim while a bundle handoff is active", async () => {
    const store = createStore();
    const router = new Router(store);
    await router.handle({
      type: "registerCodexSession",
    ...
    const bundle = await router.handle({
      type: "claimReminderBundle",
      protocolVersion: PREMIND_PROTOCOL_VERSION,
      payload: { sessionId: "cross-path" },
    });
    assert.equal(bundle.ok, true);
    const generic = await router.handle({
      type: "claimReminder",
      protocolVersion: PREMIND_PROTOCOL_VERSION,
      payload: { sessionId: "cross-path", boundary: "stop" },
    });
