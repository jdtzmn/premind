...
function parseJson(input, label) {
  try {
    ...
    throw new Error(`Codex returned invalid JSON for ${label}`, { cause: error })
  }
}
...
try {
  ...
  assert.ok(fs.existsSync(capturePath), "Interactive hooks produced no capture")
  ...
  assert.ok(eventNames.includes("UserPromptSubmit"), "UserPromptSubmit did not run")
  assert.ok(eventNames.includes("Stop"), "Stop did not run")
  assert.equal(
    captures.some(
      (capture) =>
        capture.event === "Stop" && capture.stopHookActive === true,
    ),
    true,
  ...
  console.log("  trust: one-off --dangerously-bypass-hook-trust")
} finally {
  if (pluginAdded) {
