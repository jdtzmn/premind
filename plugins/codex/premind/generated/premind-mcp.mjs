...
var SUPPORTED_MCP_PROTOCOL_VERSIONS = new Set([
	"2024-11-05",
	MCP_PROTOCOL_VERSION,
]);
var CODEX_PLUGIN_DATA_DIRECTORY = "premind-premind";
var resolveCodexPluginData = (environment, runtimeDirectory) => {
	const explicit = environment.PLUGIN_DATA?.trim();
	if (explicit) return explicit;
	const cacheRoot = path6.resolve(runtimeDirectory, "../../../..");
	const pluginsRoot = path6.dirname(cacheRoot);
	if (
		path6.basename(cacheRoot) === "cache" &&
		path6.basename(pluginsRoot) === "plugins"
	) {
		return path6.join(pluginsRoot, "data", CODEX_PLUGIN_DATA_DIRECTORY);
	}
	const codexHome =
		environment.CODEX_HOME?.trim() || path6.join(os2.homedir(), ".codex");
	return path6.join(codexHome, "plugins", "data", CODEX_PLUGIN_DATA_DIRECTORY);
};
